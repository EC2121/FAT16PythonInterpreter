...
class Fat16Interpreter:
    ...
    def read_file_allocation_table(self, starting_cluster_index, is_folder):
        ...
        elif hex(next_node_index) == "0xffff":  # case end of file, the file is all in 1 cluster so we read it 
            self.read_cluster(curr_node_index, is_folder)
            return
        else:  # case next cluster index, the file is fragmented and we need to create a linked list
    ...
    def read_cluster(self, curr_node_index, is_folder):
        ...
        if is_folder:
            ...
            for i in range(len(curr_cluster) // self.dir_entry_size):  # read every entry
                file_type = curr_cluster[11 + i * self.dir_entry_size]  # get file type
                match hex(file_type):
                    case '0x0':
                        continue
                    case '0x20':
                        ...
                        self.read_file_allocation_table(cluster_index,
                                                        False)  # read again the FAT this time is_folder is false
